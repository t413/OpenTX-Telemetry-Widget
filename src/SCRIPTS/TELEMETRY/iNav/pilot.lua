local function view(data, config, modes, dir, units, labels, gpsDegMin, hdopGraph, icons, calcBearing, calcDir, VERSION, SMLCD, FLASH, FILE_PATH, text, line, rect, fill, frmt)
	...
	local TALLLCD = SMLCD and (LCD_H > 64)
	local LEFT_POS = SMLCD and 0 or 36
	local RIGHT_POS = TALLLCD and (LCD_W - 1) or SMLCD and (LCD_W - 31) or (LCD_W - 53)
	local TOP_START_Y = TALLLCD and 32 or 0
	...
	local NARROW_OSD = (RIGHT_POS - LEFT_POS) > 120 -- narrow artifical horizon
	...
	local function pitchLadder(r, adj)
		...
		if y > 15 and y < 56 then
			...
			local onlyDotted = SMLCD and not TALLLCD
			if (y1 > 15 or y2 > 15) and (y1 < 56 or y2 < 56) then
				line(x1, TOP_START_Y + y1, x2, TOP_START_Y + y2, onlyDotted and DOTTED or (adj % 10 == 0 and SOLID or DOTTED), onlyDotted and 0 or (adj > 0 and GREY_DEFAULT or 0))
				if not onlyDotted and adj % 10 == 0 and adj ~= 0 and y1 > 15 and y1 < 56 then
					text(x1 - 2, TOP_START_Y + y1 - 3, math.abs(adj), SMLSIZE + RIGHT)
				end
			end
		end
	end
	...
	local function tics(v, p)
		for i = v % 10 + 8, 56, 10 do
			if i < 31 or i > 41 then
				line(p, TOP_START_Y + i, p + 1, TOP_START_Y + i, SOLID, 0)
			end
		end
	end
	...
	roll1 = math.rad(roll)
	if data.startup == 0 and (data.telem or TALLLCD) then
		tmp = pitch - 90
		...
		if not data.showMax then -- overlay current pitch next to crosshair
			tmp2 = tmp >= 0 and (tmp < 1 and 0 or math.floor(tmp + 0.5)) or (tmp > -1 and 0 or math.ceil(tmp - 0.5))
			text(X_CNTR - (NARROW_OSD and 14 or 24), TOP_START_Y + 33, math.abs(tmp2) .. (NARROW_OSD and "" or "\64"), SMLSIZE + RIGHT)
		end
	end
	...
	if data.showHead and data.armed and data.telem and data.gpsHome ~= false and data.startup == 0 and ((SMLCD and not data.showDir) or not SMLCD) then
		...
		if home >= LEFT_POS - (SMLCD and 0 or 7) and home <= RIGHT_POS - 1 then
			tmp = TOP_START_Y + (home > X_CNTR - 15 and home < X_CNTR + 10) and 49 or 50
	...
	elseif data.showMax then
		text(LEFT_POS + 21, TOP_START_Y + 33, "\192", SMLSIZE)
		text(RIGHT_POS - 22, TOP_START_Y + 33, "\192", SMLSIZE + RIGHT)
	end
	...
	if data.showHead then
		for i = 0, 348.75, 11.25 do
			...
			if tmp >= LEFT_POS and tmp <= RIGHT_POS then
				if i % 45 == 0 then
					text(tmp - (i % 90 == 0 and 2 or 4), TOP_START_Y + 57, dir[i / 45], SMLSIZE)
				else
					line(tmp, TOP_START_Y + 62, tmp, TOP_START_Y + 63, SOLID, FORCE)
				end
			end
		end
		fill(RIGHT_POS, TOP_START_Y + 49, 6, 14, ERASE)
	end

	-- Battery and GPS overlay
	if SMLCD and not TALLLCD then
	...
	elseif not data.armed and data.startup == 0 then
		text(LEFT_POS + 19, TOP_START_Y + 24, "Spd", SMLSIZE + RIGHT)
		text(RIGHT_POS - 2, TOP_START_Y + 24, "Alt", SMLSIZE + RIGHT)
	end

	-- Flight modes
	tmp = X_CNTR - (SMLCD and 16 or 19)
	line(tmp, TOP_START_Y + 9, tmp, 15, SOLID, ERASE)
	text(tmp + 1, TOP_START_Y + 9, modes[data.modeId].t, SMLSIZE + modes[data.modeId].f)
	if data.headFree then
		text(tmp, TOP_START_Y + 9, "HF", SMLSIZE + FLASH + RIGHT)
	end
	if data.altHold then
		icons.lock(RIGHT_POS - 28, TOP_START_Y + 33)
	end

	-- Attitude part 2 (artificial horizon)
	fill(X_CNTR - 1, TOP_START_Y + 34, 3, 3, ERASE)
	...
	for x = LEFT_POS + 1, RIGHT_POS - 1, width do
		...
		if (not upsideDown and yy < 64) or (upsideDown and yy > 7) then
			...
			fill(x, TOP_START_Y + t, width, h, GREY_DEFAULT)
		end
		y = y + a * width
	end

	-- Crosshair
	local inside = NARROW_OSD and 6 or 13
	local outside = NARROW_OSD and 14 or 24
	line(X_CNTR - outside, TOP_START_Y + 35, X_CNTR - inside, TOP_START_Y + 35, SOLID, SMLCD and 0 or FORCE)
	line(X_CNTR + outside, TOP_START_Y + 35, X_CNTR + inside, TOP_START_Y + 35, SOLID, SMLCD and 0 or FORCE)
	line(X_CNTR - inside,  TOP_START_Y + 36, X_CNTR - inside, TOP_START_Y + (NARROW_OSD and 37 or 38), SOLID, SMLCD and 0 or FORCE)
	line(X_CNTR + inside,  TOP_START_Y + 36, X_CNTR + inside, TOP_START_Y + (NARROW_OSD and 37 or 38), SOLID, SMLCD and 0 or FORCE)
	line(X_CNTR - 1,       TOP_START_Y + 35, X_CNTR + 1,      TOP_START_Y + 35, SOLID, SMLCD and 0 or FORCE)
	if SMLCD then
		lcd.drawPoint(X_CNTR, TOP_START_Y + 34, 0)
		lcd.drawPoint(X_CNTR, TOP_START_Y + 36, 0)
	else
		line(X_CNTR, TOP_START_Y + 34, X_CNTR, TOP_START_Y + 36, SOLID, FORCE)
	end

	-- Heading part 2
	if data.showHead then
		line(X_CNTR - 9,  TOP_START_Y + 56, X_CNTR + 10, TOP_START_Y + 56, SOLID, ERASE)
		line(X_CNTR - 10, TOP_START_Y + 56, X_CNTR - 10, TOP_START_Y + 63, SOLID, ERASE)
		text(X_CNTR - 9,  TOP_START_Y + 57, "      ", SMLSIZE + data.telemFlags)
		text(X_CNTR + 11, TOP_START_Y + 57, math.floor(data.heading + 0.5) % 360 .. "\64", SMLSIZE + RIGHT + data.telemFlags)
		if not SMLCD then
			lcd.drawRectangle(X_CNTR - 11, TOP_START_Y + 55, 23, 10, FORCE)
		end
	end

	-- Speed
	line(LEFT_POS, TOP_START_Y + 8, LEFT_POS, LCD_H, SOLID, FORCE) --left border
	tics(data.speed, LEFT_POS + 1)
	line(LEFT_POS + 1, TOP_START_Y + 32, LEFT_POS + 18, TOP_START_Y + 32, SOLID, ERASE)
	text(LEFT_POS + 1, TOP_START_Y + 33, "      ", SMLSIZE + data.telemFlags)
	tmp = data.showMax and data.speedMax or data.speed
	text(LEFT_POS + 19, TOP_START_Y + 33, data.startup == 0 and (tmp >= 99.5 and math.floor(tmp + 0.5) or frmt("%.1f", tmp)) or "Spd", SMLSIZE + RIGHT + data.telemFlags)
	lcd.drawRectangle(LEFT_POS, TOP_START_Y + 31, 20, 10, FORCE)

	-- Altitude
	line(RIGHT_POS, TOP_START_Y + 8, RIGHT_POS, LCD_H, SOLID, FORCE) --right border
	tics(data.altitude, RIGHT_POS - 2)
	line(RIGHT_POS - 21, TOP_START_Y + 32, RIGHT_POS, TOP_START_Y + 32, SOLID, ERASE)
	tmp = data.showMax and data.altitudeMax or data.altitude
	text(RIGHT_POS - 21, TOP_START_Y + 33, "       ", SMLSIZE + ((not data.telem or tmp + 0.5 >= config[6].v) and FLASH or 0))
	text(RIGHT_POS,      TOP_START_Y + 33, data.startup == 0 and (math.floor(tmp + 0.5)) or "Alt", SMLSIZE + RIGHT + ((not data.telem or tmp + 0.5 >= config[6].v) and FLASH or 0))
	lcd.drawRectangle(RIGHT_POS - 22, TOP_START_Y + 31, 23, 10, FORCE)

	-- Variometer
	if config[7].v % 2 == 1 then
		line(RIGHT_POS + (SMLCD and 4 or 6), TOP_START_Y + 8, RIGHT_POS + (SMLCD and 4 or 6), TOP_START_Y + 63, SOLID, FORCE)
		line(RIGHT_POS + 1, TOP_START_Y + 35, RIGHT_POS + (SMLCD and 3 or 5), TOP_START_Y + 35, SMLCD and DOTTED or SOLID, SMLCD and 0 or GREY_DEFAULT)
		if data.armed then
			...
			local y1 = 36 - (tmp * 11)
			local y2 = 36 - (tmp * 9)
			line(RIGHT_POS + 1, TOP_START_Y + y1 - 1, RIGHT_POS + (SMLCD and 3 or 5), TOP_START_Y + y2 - 1, SOLID, FORCE)
			line(RIGHT_POS + 1, TOP_START_Y + y1, RIGHT_POS + (SMLCD and 3 or 5), TOP_START_Y + y2, SOLID, FORCE)
		end
	end
	...
	if TALLLCD then
		fill(LEFT_POS, 0, LCD_W, TOP_START_Y, ERASE)
		--box 1: fuel & voltage
		local w = LCD_W / 3
		tmp = (not data.telem or data.cell < config[3].v or (data.showFuel and config[23].v == 0 and data.fuel <= config[17].v)) and FLASH or 0
		text(w - 8,  8, data.showFuel and data.fuel or (data.crsf and data.fuelRaw or data.fuel),     DBLSIZE + RIGHT + tmp)
		text(w - 6, 18, data.showFuel and "%"       or (data.fUnit[data.crsf and 1 or config[23].v]), SMLSIZE + LEFT + tmp)
		text(w - 8, 24, frmt(config[1].v == 0 and "%.2f" or "%.1f", config[1].v == 0 and (data.showMax and data.cellMin or data.cell) or (data.showMax and data.battMin or data.batt)), DBLSIZE + RIGHT + tmp)
		text(w - 6, 32, "V", SMLSIZE + LEFT + tmp)
		line(w, 10, w, TOP_START_Y + 6, SOLID, FORCE)

		--box 2: current, home dist, arrow
		text(w + 4, 8, frmt("%.1f", data.showMax and data.currentMax or data.current), MIDSIZE + LEFT + data.telemFlags)
		text(lcd.getLastRightPos(), 14, "A", SMLSIZE + LEFT + tmp)

		--box 3 txmit power & rssi
		text(X_CNTR + 8, 12, data.tpwr < 1000 and data.tpwr .. "mW" or data.tpwr * 0.001 .. "W", 0 + LEFT)
		--TODO show LQ/rssi for non-crossfire systems

		--box 4 gps
		text(LCD_W,  11, data.satellites % 100, MIDSIZE + RIGHT + data.telemFlags)
		icons.gps(lcd.getLastLeftPos() - 10, 15)
		text(LCD_W, 26, config[16].v == 0 and frmt("%.5f", data.gpsLatLon.lat) or gpsDegMin(data.gpsLatLon.lat, true), SMLSIZE + RIGHT)
		text(LCD_W, 32, config[16].v == 0 and frmt("%.5f", data.gpsLatLon.lon) or gpsDegMin(data.gpsLatLon.lon, false), SMLSIZE + RIGHT)
	end
